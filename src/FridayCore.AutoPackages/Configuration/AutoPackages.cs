...
namespace FridayCore.Configuration
{
    internal static class AutoPackages
    {
        ...
        private static DirectoryInfo GetDirectory()
        {
            ...
            var autoPackagesFolder = settingValue.Length > 2 && settingValue[1] == ':'
                ? settingValue
                : HostingEnvironment.MapPath(settingValue);
            var packagesFolder = Settings.PackagePath.Length > 2 && Settings.PackagePath[1] == ':'
                ? Settings.PackagePath
                : HostingEnvironment.MapPath(Settings.PackagePath);
            if (string.Equals(packagesFolder, autoPackagesFolder, StringComparison.OrdinalIgnoreCase))
            {
                throw new ConfigurationException(
                    $"The FridayCore.AutoPackages extension is configured incorrectly. The setting value is \"{settingValue}\" which is equal to PackagePath setting value which is not supported.");
            }
